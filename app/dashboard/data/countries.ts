...
const countriesByCode = new Map(
    countriesData.map((country) => [country.countryCode, country]),
);

const countriesByCallingCodeLength = [...countriesData].sort(
    (a, b) =>
        normalizeDigits(b.countryCallingCode).length -
        normalizeDigits(a.countryCallingCode).length,
);

export const getCountryByCode = (countryCode: string) =>
    countriesByCode.get(countryCode.toUpperCase()) ?? null;

export const getCountryCallingCode = (countryCode: string) => {
    const country = getCountryByCode(countryCode);
    const fallback = getCountryByCode(DEFAULT_COUNTRY_CODE);
    return normalizeDigits(country?.countryCallingCode ?? fallback?.countryCallingCode ?? "1");
};

export const searchCountries = (query: string, limit = 80) => {
    ...
    const filtered = countriesData.filter((country) => {
        ...
        const nameMatch = country.countryNameEn.toLowerCase().includes(trimmed);
        const isoMatch = country.countryCode.toLowerCase().includes(trimmed);
        const localNameMatch = country.countryNameLocal
            .toLowerCase()
            .includes(trimmed);
        const callingCodeDigits = normalizeDigits(country.countryCallingCode);
        const dialingMatch = digits.length > 0
            ? callingCodeDigits.includes(digits)
            : `+${callingCodeDigits}`.includes(trimmed);

        return nameMatch || isoMatch || localNameMatch || dialingMatch;
    });

    return filtered.slice(0, limit);
};

export const splitPhoneByCountryCode = (phoneValue: string) => {
    const digits = normalizeDigits(phoneValue);
    if (!digits) return null;

    const match = countriesByCallingCodeLength.find((country) => {
        const callingCode = normalizeDigits(country.countryCallingCode);
        return digits.startsWith(callingCode);
    });

    if (!match) return null;

    const callingCode = normalizeDigits(match.countryCallingCode);
    return {
        countryCode: match.countryCode,
        callingCode,
        localNumber: digits.slice(callingCode.length),
    };
};
